...
void erase_vector_keys_present_in_set(std::vector<Key>&);
...
void turn_on_update_flag(bool, long);
...
void generate_workload() {
    ...
    std::set<long> tmp_delete_pool_set = std::set<long>();
    //Preload
    if (load_from_existing_workload) {
        ...
        //load from existing workload file: NOTE: ONLY CONSIDERS INSERTS AND DELETE!!!
        if (fin.good()) {
            std::vector<std::string> splits;   
            std::string line;
            std::string command;
            while (getline(fin, line)) {
                ...
                if(command == "I") {
                    ...
                    if (tmp_insert_pool_set.find(key) != tmp_insert_pool_set.end()) {
                        tmp_insert_pool_set.erase(key);
                        //global_insert_pool.erase(std::remove(global_insert_pool.begin(), global_insert_pool.end(), key), global_insert_pool.end());
                        //tmp_delete_pool_set.insert(key); Tracking deletion
                    } else {
                        std::cerr << "Attempted to delete a non-existing key: " << key << std::endl;   
                    }
                }
            }
        sorted = false;
    ...
    int _update_flag_turned_on_time = 0;
    ...
    std::cout << "Insert Count: " << insert_count << std::endl;
    std::vector<std::chrono::milliseconds> durations1;
    while (_total_operation_count < total_operation_count) {
        if(_total_operation_count % 50000 == 0){
            std::cout << "Operation Finished: " << _total_operation_count << "   Delete Finished: " << _point_delete_count <<"   PQ Finished: "<<_point_query_count<< "   Update Finished: " << _update_count <<std::endl;
        }
	    ...
	    if (update_renew_flag) {
	        if(updateIndexGenerator != nullptr){
                if(_update_flag_turned_on_time > _update_count*0.2){
                    if(_update_flag_turned_on_time> 5000){
                        std::cout << "renew update generator    --- update_flag_turned_on_time:" << _update_flag_turned_on_time << std::endl;
                        }
                    index_mapping = updateIndexGenerator->index_mapping;
                    delete updateIndexGenerator;
                    updateIndexGenerator = new Generator(update_dist, 0, global_insert_pool.size() - 1, update_norm_mean_percentile*global_insert_pool.size(), update_norm_stddev*global_insert_pool.size()/scaling_ratio, update_beta_alpha, update_beta_beta, update_zipf_alpha, global_insert_pool.size(), update_global_index_mapping);
                    _update_flag_turned_on_time=0;
                }
            ...
            do{
                if(c > 3){
                    // std::cout << "U : C > 3 for : " << c << std::endl;
                }
                index = updateIndexGenerator->getNext();
                c++;
                }
            while(tmp_delete_pool_set.find(index) != tmp_delete_pool_set.end() || (index >= insert_pool.size() ? _insert_count >= insert_count: false ));
...
// auto start1 = std::chrono::high_resolution_clock::now();
        ...
        // durations1.push_back(duration1); // Store duration in the vector
                ...
                fp << "I " << key << " " << value << std::endl;
                _insert_count++;
                // std::cout<< "Update count: " << ud << std::endl;
            ...
            if (insert_count == point_delete_count && _insert_count == insert_count && _point_delete_count == point_delete_count - 1 && _update_count < update_count ) {
                ...
                if(tmp_delete_pool_set.size() >=  insert_pool.size()*0.2 ){
                        std::cout<< "D : Size > 0.2 : Rearrange List" << tmp_delete_pool_set.size() << std::endl;
                    erase_vector_keys_present_in_set(insert_pool);
                    // _update_il_count++;
                    tmp_delete_pool_set = std::set<long>();
                    if(existingPointLookupIndexGenerator != nullptr && point_query_count > 0){
                        ...
                        std::cout << "D: Regenerating existingPointLookupIndexGenerator --- "<< tmp_delete_pool_set.size() << std::endl;
                ...
                do{
                    if(c==3){
                        std::cout<< "D: C==3 : 1 Rearrange List" << tmp_delete_pool_set.size() << std::endl;
                        erase_vector_keys_present_in_set(insert_pool);
                        // _update_il_count++;
                        tmp_delete_pool_set = std::set<long>();
                        if(existingPointLookupIndexGenerator != nullptr){
                        std::vector<int> index_mapping = existingPointLookupIndexGenerator->index_mapping;
                        delete existingPointLookupIndexGenerator;
                        existingPointLookupIndexGenerator = new Generator(existing_point_lookup_dist, 0, insert_pool.size() - 1, existing_point_lookup_norm_mean_percentile*insert_pool.size(), existing_point_lookup_norm_stddev*insert_pool.size()/scaling_ratio, existing_point_lookup_beta_alpha, existing_point_lookup_beta_beta, existing_point_lookup_zipf_alpha, insert_pool.size(), index_mapping);
                    }}
                    index = (long)(rand() % insert_pool_size);
                    c++;
                    }
                    while(tmp_delete_pool_set.find(index) !=  tmp_delete_pool_set.end());
                Key key = insert_pool[index];
                //std::cout <<"Point Delete" << key << std::endl;
                global_insert_pool_set.erase(key);
                insert_pool[index].SetEmpty();
                tmp_delete_pool_set.insert(index);
    ...
    std::cout << "insert list reorgnize count:" << _update_il_count << std::endl;

    // Output total time
    std::cout << "Total Sort Insert Update time: " << totalDuration.count() << " milliseconds" << std::endl;

}
// Function to determine if a Key object is empty
bool isKeyEmpty(const Key& key) {
    return key.empty_flag_;
}

// void erase_vector_keys_present_in_set(std::vector<Key>& insert_pool) {
    
//     // Use std::remove_if to move empty Key objects to the end
//     auto it = std::remove_if(insert_pool.begin(), insert_pool.end(), isKeyEmpty);
//     // Erase the empty Key objects from the vector
//     insert_pool.erase(it, insert_pool.end());
//     // std::cout<< "ReOrg Insert Pool"<< std::endl;
...
void erase_vector_keys_present_in_set(std::vector<Key>& insert_pool) {
    ...
    for (size_t i = 0; i < insert_pool.size(); ++i) {
        // If the element is not empty, swap it with the next non-empty element
        if (!insert_pool[i].empty_flag_) {
            std::swap(insert_pool[i], insert_pool[nonEmptyIndex]);
            ++nonEmptyIndex;
        }
    }
    // Erase the empty elements from the end of the vector
    insert_pool.erase(insert_pool.begin() + nonEmptyIndex, insert_pool.end());
}
...
int get_choice(long insert_pool_size, long insert_count, long update_count, long point_delete_count, long range_delete_count, long point_query_count, long range_query_count, long _insert_count, long _update_count, long _point_delete_count, long _range_delete_count, long _point_query_count, long _range_query_count) {
    ...
    // printf("insert_count - _insert_count %ld -------/n", insert_count - _insert_count);
...
void turn_on_update_flag(bool update_flag, int _update_flag_turned_on_time){
    update_flag = true;
    _update_flag_turned_on_time++;
}
